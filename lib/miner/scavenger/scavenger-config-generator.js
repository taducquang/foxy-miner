...
class ScavengerConfigGenerator {
  ...
  generate({ minerConfig, minerIndex = null }) {
    let listenAddr = this.config.listenAddr;
    if (this.config.listenHost === '0.0.0.0') {
      listenAddr = `127.0.0.1:${this.config.listenPort}`;
    }
    let url = `http://${listenAddr}`;
    if (minerIndex !== null) {
      url += `/${minerIndex + 1}`;
    }
    ...
    const generatedConfig = {
      plot_dirs: plotDirs,
      url,
      hdd_reader_thread_count: this.getFromMinerConfig(minerConfig, { key: 'hdd_reader_thread_count', defaultValue: 0 }),
      hdd_use_direct_io: this.getFromMinerConfig(minerConfig, { key: 'useHddDirectIo', defaultValue: true }),
      hdd_wakeup_after: this.getFromMinerConfig(minerConfig, { key: 'hdd_wakeup_after', defaultValue: 0 }),

      cpu_threads: this.getFromMinerConfig(minerConfig, { key: 'cpu_threads', defaultValue: 0 }),
  ...
  updateConfig({ configYaml, minerConfig, minerIndex = null }) {
    ...
    if (plotDirs !== null && (!Array.isArray(config.plot_dirs) || !isEqual(config.plot_dirs.sort(), plotDirs.sort()))) {
      config.plot_dirs = plotDirs;
    }

    const hdd_reader_thread_count = this.getFromMinerConfig(minerConfig, { key: 'hdd_reader_thread_count', defaultValue: 0 });
    if (hdd_reader_thread_count !== null && config.hdd_reader_thread_count !== 0) {
      config.hdd_reader_thread_count = hdd_reader_thread_count;
    }

    const hdd_wakeup_after = this.getFromMinerConfig(minerConfig, { key: 'hdd_wakeup_after', defaultValue: 0 });
    if (hdd_wakeup_after !== null && config.hdd_wakeup_after !== 0) {
      config.hdd_wakeup_after = hdd_wakeup_after;
    }
    ...
    const cpu_threads = this.getFromMinerConfig(minerConfig, { key: 'cpu_threads', defaultValue: 0 });
    if (cpu_threads !== null && config.cpu_threads !== 0) {
